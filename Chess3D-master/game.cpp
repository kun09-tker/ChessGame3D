...
void Game::tryMovement(int vaoId, int caseX, int caseY) {
    ...
    if (current_piece != nullptr) {
        std::cout << "--> Player " << turn << " selected vao" << vaoId << "("
                  << current_piece->getPosition()[0] << ";" << current_piece->getPosition()[1]
                  << ") and clicked on cell (" << caseX << "," << caseY << ")" << std::endl;
        ...
        std::cout << "--> Player " << turn << " selected vao" << vaoId << " and clicked on cell ("
                  << caseX << "," << caseY << ") VAO NULLPOINTER" << std::endl;
}
...
void Game::saveToFile() {
    ...
    for (Piece *piece : player1.getPieces())
        myfile << "1 " << piece->getName() << " " << piece->getPosition()[0] << " "
               << piece->getPosition()[1] << std::endl;

    for (Piece *piece : player2.getPieces())
        myfile << "2 " << piece->getName() << " " << piece->getPosition()[0] << " "
               << piece->getPosition()[1] << std::endl;
...
bool Game::checkMate(Player player, Player opponent, std::vector<Piece *> threateningPieces) {
    ...
    // On regarde si on peut prendre la pièce menaçante directement (efficace pour les cavaliers et
    // pions)
    ...
    for (unsigned int i = 0; i < player.getPieces().size(); i++) {
        ...
        for (unsigned int j = 0; j < player.getPieces()[i]->getAvailableMovements().size(); j++) {
            ...
            if (check(player, opponent, kingPos).size() == 0) {
                ...
                //                          << tempPos[1] << ") en " <<
                //                          " (" << possibleMovement[0] << ":"<< possibleMovement[1]
